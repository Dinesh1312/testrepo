...
public class Campaign extends WebDriverImpl{
	...
	public void beforeClass() throws Exception {
		...
		wait(3000);
		Assert.assertEquals(driver.getCurrentUrl(), "https://app"+loginData.getENV()+".experience.com/user/account/dashboard");
		...
		Assert.assertEquals(driver.findElement(By.xpath("//div[@class='drawerProfile']/label")).getText(),"PageValidation AccMngr");
		Assert.assertEquals(driver.findElement(By.xpath("//div[@class='drawerProfile']//following::label")).getText(),"Account Manager");
	...
	@Test()
	public void tc01_verifyAccountManagerLandingPage() throws Exception {
		...
		String count=(driver.findElement(By.id("campaign_dashboard_metricsBar_Total Surveys Sent")).getText()).replace(",", ".");
		Assert.assertTrue(Double.parseDouble(count)>0);
	...
	public void tc03_verifyManualSurveyFields() throws Exception {
		try {
			...
			click(driver.findElement(By.xpath("//a[@title='Automation']/following::span[@id='campaign_campaignsList_list_threeDot'][1]")));
	...
	public void tc04_verifyViewAnalytics() throws Exception {
		...
		wait(3000);
		Assert.assertTrue(driver.findElement(By.id("undefined_public_campaignName")).isEnabled() && driver.findElement(By.id("undefined_internal_campaignName")).isEnabled());
		Assert.assertTrue((driver.findElements(By.xpath("//li[@id='campaign_sideBar_campaignDetails_li']//div/span")).size())==8);
	}
	...
	public void tc05_verifyOverview() throws Exception {
		...
		List<String> preprodExpectedAnalyticsList = Arrays.asList("Overview","Question Summary","Reviews","Incomplete Surveys");
		if (loginData.getENV().equals(".preprod")) {
			Assert.assertEquals(analyticsList,preprodExpectedAnalyticsList);
		}else {
		...
		String count=(driver.findElement(By.id("campaign_overview_primaryStatus_totalResponses")).getText()).replace(",", ".");
		Assert.assertTrue(Double.parseDouble(count)>0);
